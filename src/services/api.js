...
export const getPopularMovies = async () => {
  ...
  const response = await fetch(`${BASE_URL}/movie/popular?language=${lang}`, {
    headers: {
...
export const searchMovies = async (query) => {
  const lang = i18next.language
  const response = await fetch(`${BASE_URL}/search/movie?language=${lang}&query=${encodeURIComponent(query)}`, {
    headers: {
      Authorization: `Bearer ${API_TOKEN}`,
    },
  })
